...
<form action="functions/registroturma.php" method="post">
    <table style="box-shadow: 0 4px 8px 0 rgba(0, 0,0,0.08);  margin-left: 250px; transform: translate(0,30px);" >
     
        <tr >
        <h3 style="color: #013f86; text-align: center;">REGISTRO DE Turma</h3>
            <td>Nome</td>
...
<button style="transform: translate(0,40px);padding: 10px;   background: #777; color: #fff; border-radius: 5px;    border: 0;cursor: pointer;width: 200px;">
 <a style="text-decoration: none; color: #fff;" href="pdfturma.php"><i class='bx bxs-edit'></i> pdf</a></button>
...
<div class="search" style="margin-left: 250px;">
    ...
    <?php 
   $sql="SELECT * FROM turma  AS t INNER JOIN sala AS s ON t.codprofessor=s.codigo ";
   ...
   while ($row=$query->fetch_assoc()) {
    ...
    <tr align="center">
          <td><?= $row['codigo'] ?></td>
         <td><?= $row['nome'] ?></td>
        <td><?= $row['codprofessor'] ?></td>
        <td><?= $row['nome'] ?></td>
        <td><?= $row['cod_ano'] ?></td>
        <td align="center">
            <a style="text-decoration: none; color: #777;" href="functions/editurma.php?editar=<?= $row['codigo']; ?>"><i class='bx bxs-edit'></i> Editar</a>
            <a style="text-decoration: none; color: #777;"  href="functions/deleteturma.php?eleminar=<?= $row['codigo']; ?>"><i class='bx bxs-delete'></i></i><i class='bx bx-x'></i>Deletar</a>
        </td>
